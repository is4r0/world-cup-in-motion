...
function isFinalStage(match) {
  return String(match?.stage || '').toUpperCase() === 'FINAL';
}

function finalChampionText(match) {
  const home = match?.score?.home;
  const away = match?.score?.away;
  if (home == null || away == null) return 'Champion';
  if (home === away) return 'Final decided';
  return home > away ? liveTeamName(match.homeTeam) : liveTeamName(match.awayTeam);
}

function finalScorerSummary(match) {
  const home = normalizeName(match?.homeTeam?.name || '');
  const away = normalizeName(match?.awayTeam?.name || '');
  if ((home.includes('spain') && away.includes('argentina')) || (home.includes('argentina') && away.includes('spain'))) {
    return "Ferran Torres 106' / extra-time winner";
  }
  return scorerSummary(match);
}

function setPostFinalAwardsVisible(isVisible) {
  const awards = document.getElementById('post-final-awards');
  if (awards) awards.classList.toggle('is-hidden', !isVisible);
}
...
function updateSmallLiveBar() {
  ...
  if (!match) {
    ...
    setPostFinalAwardsVisible(false);
    return;
  }

  const isLive = !!active;
  const isResult = !isLive && !!result;
  const isFinalResult = isResult && isFinalStage(match);
  ...
  bar.classList.toggle('is-final-result', isFinalResult);
  bar.classList.remove('is-hidden');
  setPostFinalAwardsVisible(isFinalResult);
  if (state) state.textContent = isLive ? liveMinuteText(match) : isFinalResult ? 'Champs' : isResult ? 'FT' : 'Next';
  ...
  if (stage) stage.textContent = isFinalResult
    ? `${finalChampionText(match)} lift the trophy / Final`
    : isResult ? `${matchWinnerText(match)} / ${match.stage ? titleCaseWords(String(match.stage).replaceAll('_', ' ')) : 'Result'}` : matchStageText(match);
  if (homeName) homeName.textContent = liveTeamName(match.homeTeam);
  if (awayName) awayName.textContent = liveTeamName(match.awayTeam);
  if (scorers) scorers.textContent = isFinalResult ? finalScorerSummary(match) : (isLive || isResult) ? scorerSummary(match) : 'Goal details on kickoff';
  if (nextLabel) {
    nextLabel.textContent = isFinalResult
      ? `${finalChampionText(match).toUpperCase()} WON ${matchScoreText(match)}`
      : next
...
const PLAYER_STATS = {
  goals: [['Kylian Mbappe','France','KM',10], ['Lionel Messi','Argentina','LM',8], ['Jude Bellingham','England','JB',7], ['Erling Haaland','Norway','EH',7], ['Harry Kane','England','HK',6], ['Ousmane Dembele','France','OD',6], ['Mikel Oyarzabal','Spain','MO',5], ['Ferran Torres','Spain','FT',5], ['Julian Quinones','Mexico','JQ',4], ['Vinicius Junior','Brazil','VJ',4]],
  assists: [['Michael Olise','France','MO',7], ['Lionel Messi','Argentina','LM',4], ['Bruno Guimaraes','Brazil','BG',4], ['Brahim Diaz','Morocco','BD',4], ['Martin Odegaard','Norway','MO',4], ['Kylian Mbappe','France','KM',3], ['Andreas Schjelderup','Norway','AS',3], ['Roberto Alvarado','Mexico','RA',3], ['Alexander Isak','Sweden','AI',3], ['Florian Wirtz','Germany','FW',3]],
  xg: [['Kylian Mbappe','France','KM','7.10'], ['Lionel Messi','Argentina','LM','5.90'], ['Erling Haaland','Norway','EH','4.43'], ['Jude Bellingham','England','JB','4.02'], ['Harry Kane','England','HK','3.74'], ['Ousmane Dembele','France','OD','3.51'], ['Mikel Oyarzabal','Spain','MO','3.20'], ['Ferran Torres','Spain','FT','3.00'], ['Ismaila Sarr','Senegal','IS','2.90'], ['Vinicius Junior','Brazil','VJ','2.80']],
  shots: [['Lionel Messi','Argentina','LM',38], ['Kylian Mbappe','France','KM',36], ['Erling Haaland','Norway','EH',20], ['Ousmane Dembele','France','OD',20], ['Jude Bellingham','England','JB',19], ['Harry Kane','England','HK',18], ['Mikel Oyarzabal','Spain','MO',18], ['Ferran Torres','Spain','FT',17], ['Ismaila Sarr','Senegal','IS',17], ['Julian Quinones','Mexico','JQ',14]],
  chances: [['Michael Olise','France','MO',25], ['Lionel Messi','Argentina','LM',24], ['Lamine Yamal','Spain','LY',20], ['Dani Olmo','Spain','DO',18], ['Rodri','Spain','RD',17], ['Brahim Diaz','Morocco','BD',15], ['Martin Odegaard','Norway','MO',15], ['Bruno Guimaraes','Brazil','BG',14], ['Alexis Mac Allister','Argentina','AM',14], ['Kylian Mbappe','France','KM',13]],
  clean: [['Unai Simon','Spain','US',7], ['Emiliano Martinez','Argentina','EM',4], ['Mike Maignan','France','MM',4], ['Jordan Pickford','England','JP',4], ['Orlando Gill','Paraguay','OG',3], ['Yassine Bounou','Morocco','YB',3], ['Gregor Kobel','Switzerland','GK',3], ['Yann Sommer','Switzerland','YS',3], ['Diogo Costa','Portugal','DC',2], ['Alisson Becker','Brazil','AB',2]],
  saves: [['Emiliano Martinez','Argentina','EM',29], ['Orlando Gill','Paraguay','OG',23], ['Gregor Kobel','Switzerland','GK',20], ['Eloy Room','Curacao','ER',20], ['Diogo Costa','Portugal','DC',19], ['Vozinha','Cape Verde','VZ',18], ['Jordan Pickford','England','JP',17], ['Mike Maignan','France','MM',16], ['Unai Simon','Spain','US',15], ['Yassine Bounou','Morocco','YB',15]],
  tackles: [['Rodri','Spain','RD',36], ['Declan Rice','England','DR',35], ['Cristian Romero','Argentina','CR',32], ['Aurelien Tchouameni','France','AT',30], ['Alexis Mac Allister','Argentina','AM',29], ['Pau Cubarsi','Spain','PC',29], ['Marc Cucurella','Spain','MC',27], ['John Stones','England','JS',25], ['Leandro Paredes','Argentina','LP',25], ['Sofyan Amrabat','Morocco','SA',22]],
  interceptions: [['Pau Cubarsi','Spain','PC',24], ['Cristian Romero','Argentina','CR',22], ['Rodri','Spain','RD',20], ['John Stones','England','JS',19], ['William Saliba','France','WS',18], ['Marc Cucurella','Spain','MC',18], ['Nicolas Otamendi','Argentina','NO',17], ['Declan Rice','England','DR',16], ['Jules Kounde','France','JK',16], ['Achraf Hakimi','Morocco','AH',14]],
  motm: [['Rodri','Spain','RD',5], ['Lionel Messi','Argentina','LM',5], ['Pau Cubarsi','Spain','PC',4], ['Jude Bellingham','England','JB',4], ['Lamine Yamal','Spain','LY',4], ['Kylian Mbappe','France','KM',3], ['Michael Olise','France','MO',3], ['Ferran Torres','Spain','FT',3], ['Mikel Oyarzabal','Spain','MO',3], ['Emiliano Martinez','Argentina','EM',3]],
  yellow: [['Leandro Paredes','Argentina','LP',3], ['Nicolas Tagliafico','Argentina','NT',3], ['Cristian Romero','Argentina','CR',3], ['Rodrigo De Paul','Argentina','RD',2], ['Lamine Yamal','Spain','LY',2], ['Rodri','Spain','RD',2], ['Declan Rice','England','DR',2], ['Breel Embolo','Switzerland','BE',2], ['Michael Olise','France','MO',2], ['Granit Xhaka','Switzerland','GX',2]],
  red: [['Enzo Fernandez','Argentina','EF',1], ['Jarell Quansah','England','JQ',1], ['Nathan Ngoy','Belgium','NN',1], ['Tarik Muharemovic','Bosnia','TM',1], ['Agustin Canobbio','Uruguay','AC',1], ['Piero Hincapie','Ecuador','PH',1], ['Folarin Balogun','USA','FB',1], ['Breel Embolo','Switzerland','BE',1], ['Miguel Almiron','Paraguay','MA',1], ['Achraf Hakimi','Morocco','AH',1]]
};
...
function completeKnockoutMatches(matches) {
  const finalStages = new Set(KNOWN_KNOCKOUT_RESULTS.map(match => match.stage));
  const completed = matches.filter(match => !finalStages.has(match.stage));
  KNOWN_KNOCKOUT_RESULTS.forEach(known => {
    const liveMatch = matches.find(match => match.stage === known.stage);
    completed.push(hasCompleteScore(liveMatch) ? liveMatch : known);
  });
  return completed.sort((a, b) => new Date(a.utcDate || 0) - new Date(b.utcDate || 0));
}
...
function renderBracketView() {
  const knockoutMatches = completeKnockoutMatches(LIVE_DATA.matches.filter(m => m.stage && m.stage !== 'GROUP_STAGE'));
